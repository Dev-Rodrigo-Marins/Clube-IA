...
// Pinos de conexão da camera OV7670 com a ESP32
...
// Pinos de acionamento dos leds
const int ledAzul = 23;
const int ledVerde = 2;
const int ledAmarelo = 18;
const int ledVermelho = 19;

const char* serverUrl = "http://192.168.0.105:5000/button_pressed";  // Substitua 192.168.0.105 pelo IP do seu computador onde esta rodando Flask
const int buttonPin = 5;  // Ajuste o pino conforme sua configuração para colocar o pino de start(touch)

#define ssid1        "Nome da sua REDE WIFI"  // Nome da rede Wifi
#define password1    "Senha da sua rede WIFI" // Senha da rede Wifi
    ...
    if (client) 
    {
        ...
        while (client.connected()) 
        {
            if (client.available()) 
            {
                ...
                {
                    client.println();
                    client.println("<html><body><h1>Residuo não classificado!!</h1></body></html>");
                    digitalWrite(ledVermelho, HIGH);
                    delay(500);
                    digitalWrite(ledVermelho, LOW);
                    digitalWrite(ledAmarelo, HIGH);
